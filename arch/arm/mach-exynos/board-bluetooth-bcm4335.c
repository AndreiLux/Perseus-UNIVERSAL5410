...
static unsigned int bt_uart_on_table[][4] = {
	{EXYNOS5410_GPA0(0), 2, 2, S3C_GPIO_PULL_NONE},
	{EXYNOS5410_GPA0(1), 2, 2, S3C_GPIO_PULL_NONE},
	{EXYNOS5410_GPA0(2), 2, 2, S3C_GPIO_PULL_NONE},
	{EXYNOS5410_GPA0(3), 2, 2, S3C_GPIO_PULL_NONE},
};
...
void bt_uart_rts_ctrl(int flag)
{
	...
	if (flag) {
		/* BT RTS Set to HIGH */
		s3c_gpio_cfgpin(EXYNOS5410_GPA0(3), S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(EXYNOS5410_GPA0(3), S3C_GPIO_PULL_NONE);
		gpio_set_value(EXYNOS5410_GPA0(3), 1);
	} else {
		/* restore BT RTS state */
		s3c_gpio_cfgpin(EXYNOS5410_GPA0(3), S3C_GPIO_SFN(2));
		s3c_gpio_setpull(EXYNOS5410_GPA0(3), S3C_GPIO_PULL_NONE);
	}
}
...
static int bcm4335_bluetooth_probe(struct platform_device *pdev)
{
	...
	s3c_gpio_setpull(EXYNOS5410_GPX2(1), S3C_GPIO_PULL_NONE);
