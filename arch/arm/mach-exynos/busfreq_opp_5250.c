			...
			struct opp **mif_opp, struct opp **int_opp)
{
	...
	right0_load = div64_u64(ppmu_load[PPMU_RIGHT] * data->curr_freq[PPMU_INT], data->max_freq[PPMU_INT]);
	...
	data->load_history[PPMU_RIGHT][data->index] = right0_load;
	...
	for (i = 0; i < LOAD_HISTORY_SIZE; i++) {
		...
		right0_load_average += data->load_history[PPMU_RIGHT][i];
...
int exynos5250_init(struct device *dev, struct busfreq_data *data)
{
	...
	dmc_pause_ctrl |= EXYNOS5_DMC_PAUSE_ENABLE;
	__raw_writel(dmc_pause_ctrl, EXYNOS5_DMC_PAUSE_CTRL);
