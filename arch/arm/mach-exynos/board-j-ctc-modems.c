...
static void config_cdma_modem_gpio(void)
{
	...
	unsigned gpio_usb_boot_en = cdma_modem_data.gpio_usb_boot_en;
	...
	if (gpio_rst_req_n) {
		...
		if (err) {
			...
			gpio_direction_input(gpio_rst_req_n);
	...
	if (gpio_usb_boot_en) {
		...
		if (err) {
			pr_err("fail to request gpio %s, gpio %d, errno %d\n",
				   "USB_BOOT_EN", gpio_usb_boot_en, err);
		} else {
...
static struct modemlink_dpram_data gsm_edpram_ctrl = {
	.type = EXT_DPRAM,
...
void config_gsm_modem_gpio(void)
{
	...
	if (gpio_pda_active) {
		...
		if (err) {
			if (err == -EBUSY) {
				pr_err("gpio %s is already requested, gpio %d, errno %d\n",
...
static u8 *gsm_edpram_remap_mem_region(struct sromc_cfg *cfg)
{
	...
	gsm_edpram_ctrl.base = (u8 __iomem *)dp_base;
	gsm_edpram_ctrl.size = dp_size;
