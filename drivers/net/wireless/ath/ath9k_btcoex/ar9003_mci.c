...
static void ar9003_mci_remote_reset(struct ath_hw *ah, bool wait_done)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_REMOTE_RESET, 0, payload, 16,
				wait_done, false);
	udelay(5);
}
...
static void ar9003_mci_send_lna_transfer(struct ath_hw *ah, bool wait_done)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_LNA_TRANS, 0, &payload, 1,
				wait_done, false);
}

static void ar9003_mci_send_req_wake(struct ath_hw *ah, bool wait_done)
{
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_REQ_WAKE, MCI_FLAG_DISABLE_TIMESTAMP,
...
{
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_SYS_WAKING, MCI_FLAG_DISABLE_TIMESTAMP,
				NULL, 0, wait_done, false);
}
...
static void ar9003_mci_send_lna_take(struct ath_hw *ah, bool wait_done)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_LNA_TAKE, 0, &payload, 1,
				wait_done, false);
}

static void ar9003_mci_send_sys_sleeping(struct ath_hw *ah, bool wait_done)
{
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_SYS_SLEEPING,
					       ...
					       bool wait_done)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, payload, 16, wait_done, true);
}
						...
						bool wait_done, u8 query_type)
{
	...
	*(((u8 *)payload) + MCI_GPM_COEX_B_BT_BITMAP) = query_type;
	 ...
	 */
	if (!ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, payload, 16,
				wait_done, true)) {
		if (query_btinfo)
...
void ar9003_mci_set_full_sleep(struct ath_hw *ah)
{
	...
	if (ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_ENABLE) &&
	    (mci->bt_state != MCI_BT_SLEEP) &&
	    !mci->halted_bt_gpm) {
...
void ath9k_btcoex_ar9003_mci_get_interrupt(struct ath_hw *ah, u32 *raw_intr,
			      u32 *rx_msg_intr)
{
	struct ath9k_hw_mci *mci = &ah->btcoex_hw.mci;
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_get_interrupt);
...
static void ar9003_mci_observation_set_up(struct ath_hw *ah)
{
	...
	if (mci->config & ATH_MCI_CONFIG_MCI_OBS_MCI) {
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 3, AR_GPIO_OUTPUT_MUX_AS_MCI_WLAN_DATA);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 2, AR_GPIO_OUTPUT_MUX_AS_MCI_WLAN_CLK);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 1, AR_GPIO_OUTPUT_MUX_AS_MCI_BT_DATA);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 0, AR_GPIO_OUTPUT_MUX_AS_MCI_BT_CLK);
	} else if (mci->config & ATH_MCI_CONFIG_MCI_OBS_TXRX) {
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 3, AR_GPIO_OUTPUT_MUX_AS_WL_IN_TX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 2, AR_GPIO_OUTPUT_MUX_AS_WL_IN_RX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 1, AR_GPIO_OUTPUT_MUX_AS_BT_IN_TX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 0, AR_GPIO_OUTPUT_MUX_AS_BT_IN_RX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 5, AR_GPIO_OUTPUT_MUX_AS_OUTPUT);
	} else if (mci->config & ATH_MCI_CONFIG_MCI_OBS_BT) {
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 3, AR_GPIO_OUTPUT_MUX_AS_BT_IN_TX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 2, AR_GPIO_OUTPUT_MUX_AS_BT_IN_RX);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 1, AR_GPIO_OUTPUT_MUX_AS_MCI_BT_DATA);
		ath9k_btcoex_ath9k_hw_cfg_output(ah, 0, AR_GPIO_OUTPUT_MUX_AS_MCI_BT_CLK);
	} else
		return;
					  ...
					  u8 opcode, u32 bt_flags)
{
	...
	*(((u8 *)pld) + MCI_GPM_COEX_W_BT_FLAGS + 3) = (bt_flags >> 24) & 0xFF;
	...
	return ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, pld, 16,
				       wait_done, true);
}
...
static void ar9003_mci_sync_bt_state(struct ath_hw *ah)
{
	...
	cur_bt_state = ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_REMOTE_SLEEP);
				   ...
				   u8 gpm_opcode, int time_out)
{
	...
	while (time_out > 0) {
		...
		offset = ath9k_btcoex_ar9003_mci_get_next_gpm_offset(ah, false, &more_data);
		...
		if ((gpm_type == MCI_GPM_BT_CAL_GRANT) &&
			...
			ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, payload, 16,
						false, false);
			continue;
		} else {
...
bool ar9003_mci_start_reset(struct ath_hw *ah, struct ath9k_channel *chan)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, payload,
				16, true, false);
		      ...
		      bool is_full_sleep)
{
	...
	*/
	...
	ath9k_btcoex_ar9003_mci_get_next_gpm_offset(ah, true, NULL);
...
bool ath9k_btcoex_ar9003_mci_send_message(struct ath_hw *ah, u8 header, u32 flag,
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_send_message);
...
void ar9003_mci_init_cal_req(struct ath_hw *ah, bool *is_reusable)
{
	...
	ath9k_btcoex_ar9003_mci_send_message(ah, MCI_GPM, 0, pld, 16, true, false);
...
void ath9k_btcoex_ar9003_mci_setup(struct ath_hw *ah, u32 gpm_addr, void *gpm_buf,
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_setup);

void ath9k_btcoex_ar9003_mci_cleanup(struct ath_hw *ah)
{
	/* Turn off MCI and Jupiter mode. */
	REG_WRITE(ah, AR_BTCOEX_CTRL, 0x00);
	ar9003_mci_disable_interrupt(ah);
}
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_cleanup);

u32 ath9k_btcoex_ar9003_mci_state(struct ath_hw *ah, u32 state_type)
{
	struct ath9k_hw_mci *mci = &ah->btcoex_hw.mci;
	u32 value = 0;
	u8 query_type;
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_state);
...
u32 ath9k_btcoex_ar9003_mci_get_next_gpm_offset(struct ath_hw *ah, bool first, u32 *more)
{
	struct ath9k_hw_mci *mci = &ah->btcoex_hw.mci;
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_get_next_gpm_offset);

void ath9k_btcoex_ar9003_mci_set_bt_version(struct ath_hw *ah, u8 major, u8 minor)
{
	struct ath9k_hw_mci *mci = &ah->btcoex_hw.mci;
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_set_bt_version);

void ath9k_btcoex_ar9003_mci_send_wlan_channels(struct ath_hw *ah)
{
	struct ath9k_hw_mci *mci = &ah->btcoex_hw.mci;
...
EXPORT_SYMBOL(ath9k_btcoex_ar9003_mci_send_wlan_channels);
