...
skip_tuning:
	...
	ath9k_btcoex_ath9k_hw_btcoex_disable(sc->sc_ah);
	...
	ath9k_btcoex_ath9k_hw_btcoex_enable(sc->sc_ah);
...
static void ath_mci_cal_msg(struct ath_softc *sc, u8 opcode, u8 *rx_payload)
{
	...
	case MCI_GPM_BT_CAL_REQ:
		if (mci_hw->bt_state == MCI_BT_AWAKE)
			ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_SET_BT_CAL_START);
		ath_dbg(common, MCI, "MCI State : %d\n", mci_hw->bt_state);
		break;
	case MCI_GPM_BT_CAL_GRANT:
		MCI_GPM_SET_CAL_TYPE(payload, MCI_GPM_WLAN_CAL_DONE);
		ath9k_btcoex_ar9003_mci_send_message(sc->sc_ah, MCI_GPM, 0, payload,
					16, false, true);
		break;
	default:
...
static void ath_mci_msg(struct ath_softc *sc, u8 opcode, u8 *rx_payload)
{
	...
	case MCI_GPM_COEX_VERSION_RESPONSE:
		...
		ath9k_btcoex_ar9003_mci_set_bt_version(ah, major, minor);
		break;
	case MCI_GPM_COEX_STATUS_QUERY:
		ath9k_btcoex_ar9003_mci_send_wlan_channels(ah);
		break;
...
int ath_mci_setup(struct ath_softc *sc)
{
	...
	ath9k_btcoex_ar9003_mci_setup(sc->sc_ah, mci->gpm_buf.bf_paddr,
...
void ath_mci_cleanup(struct ath_softc *sc)
{
	...
	ath9k_btcoex_ar9003_mci_cleanup(ah);
...
void ath_mci_intr(struct ath_softc *sc)
{
	...
	ath9k_btcoex_ar9003_mci_get_interrupt(sc->sc_ah, &mci_int, &mci_int_rxmsg);

	if (ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_ENABLE) == 0) {
	...
	if (mci_int_rxmsg & AR_MCI_INTERRUPT_RX_MSG_REQ_WAKE) {
		 ...
		 */
		ath9k_btcoex_ar9003_mci_send_message(ah, MCI_REMOTE_RESET, 0,
					payload, 16, true, false);
		ath9k_btcoex_ar9003_mci_send_message(ah, MCI_SYS_WAKING, 0,
					NULL, 0, true, false);

		mci_int_rxmsg &= ~AR_MCI_INTERRUPT_RX_MSG_REQ_WAKE;
		ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_RESET_REQ_WAKE);

		/*
		 * always do this for recovery and 2G/5G toggling and LNA_TRANS
		 */
		ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_SET_BT_AWAKE);
	}
	...
	if (mci_int_rxmsg & AR_MCI_INTERRUPT_RX_MSG_SYS_WAKING) {
		...
		if ((mci_hw->bt_state == MCI_BT_SLEEP) &&
		    (ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_REMOTE_SLEEP) !=
		     MCI_BT_SLEEP))
			ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_SET_BT_AWAKE);
	}
	...
	if ((mci_int & AR_MCI_INTERRUPT_RX_INVALID_HDR) ||
		...
		ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_RECOVER_RX);
		skip_gpm = true;
	}

	if (mci_int_rxmsg & AR_MCI_INTERRUPT_RX_MSG_SCHD_INFO) {
		mci_int_rxmsg &= ~AR_MCI_INTERRUPT_RX_MSG_SCHD_INFO;
		offset = ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_LAST_SCHD_MSG_OFFSET);
	}
	...
	if (mci_int_rxmsg & AR_MCI_INTERRUPT_RX_MSG_GPM) {
		...
		while (more_data == MCI_GPM_MORE) {
			...
			offset = ath9k_btcoex_ar9003_mci_get_next_gpm_offset(ah, false,
								&more_data);
