			       ...
			       enum led_brightness brightness)
{
	...
	ath9k_btcoex_ath9k_hw_set_gpio(sc->sc_ah, sc->sc_ah->led_pin, (brightness == LED_OFF));
}
...
static void ath_detect_bt_priority(struct ath_softc *sc)
{
	...
	if (ath9k_btcoex_ath9k_hw_gpio_get(sc->sc_ah, ah->btcoex_hw.btpriority_gpio))
...
static void ath9k_gen_timer_start(struct ath_hw *ah,
				  ...
				  u32 timer_period)
{
	ath9k_btcoex_ath9k_hw_gen_timer_start(ah, timer, trig_timeout, timer_period);
...
static void ath9k_gen_timer_stop(struct ath_hw *ah, struct ath_gen_timer *timer)
{
	...
	ath9k_btcoex_ath9k_hw_gen_timer_stop(ah, timer);
...
static void ath_btcoex_period_timer(unsigned long data)
{
	...
	if (btcoex->bt_wait_time > ATH_BTCOEX_RX_WAIT_TIME) {
		if (ath9k_btcoex_ar9003_mci_state(ah, MCI_STATE_NEED_FTP_STOMP) &&
	...
	ath9k_btcoex_ath9k_hw_btcoex_bt_stomp(ah, is_btscan ? ATH_BTCOEX_STOMP_ALL :
...
static int ath_init_btcoex_timer(struct ath_softc *sc)
{
	...
	btcoex->no_stomp_timer = ath9k_btcoex_ath_gen_timer_alloc(sc->sc_ah,
...
void ath9k_btcoex_handle_interrupt(struct ath_softc *sc, u32 status)
{
	...
	if (ath9k_hw_get_btcoex_scheme(ah) == ATH_BTCOEX_CFG_3WIRE)
		if (status & ATH9K_INT_GENTIMER)
			ath9k_btcoex_ath_gen_timer_isr(sc->sc_ah);
...
void ath9k_stop_btcoex(struct ath_softc *sc)
{
	...
	if (ah->btcoex_hw.enabled &&
		...
		ath9k_btcoex_ath9k_hw_btcoex_disable(ah);
		if (AR_SREV_9462(ah))
...
void ath9k_deinit_btcoex(struct ath_softc *sc)
{
	struct ath_hw *ah = sc->sc_ah;
		...
		ath9k_btcoex_ath_gen_timer_free(sc->sc_ah, sc->btcoex.no_stomp_timer);
...
int ath9k_init_btcoex(struct ath_softc *sc)
{
	...
	ath9k_btcoex_ath9k_hw_btcoex_init_scheme(ah);

	switch (ath9k_hw_get_btcoex_scheme(sc->sc_ah)) {
	case ATH_BTCOEX_CFG_NONE:
		break;
	case ATH_BTCOEX_CFG_2WIRE:
		ath9k_btcoex_ath9k_hw_btcoex_init_2wire(sc->sc_ah);
		break;
	case ATH_BTCOEX_CFG_3WIRE:
		ath9k_btcoex_ath9k_hw_btcoex_init_3wire(sc->sc_ah);
		r = ath_init_btcoex_timer(sc);
		if (r)
			return -1;
		txq = sc->tx.txq_map[WME_AC_BE];
		ath9k_btcoex_ath9k_hw_init_btcoex_hw(sc->sc_ah, txq->axq_qnum);
		...
		if (ath9k_hw_mci_is_enabled(ah)) {
			...
			ath9k_btcoex_ath9k_hw_btcoex_init_mci(ah);
		}
