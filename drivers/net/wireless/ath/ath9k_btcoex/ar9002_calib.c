				...
				bool longcal)
{
	...
	if (longcal || nfcal_pending) {
		 ...
		 * history buffer.
		 */
		if (ath9k_btcoex_ath9k_hw_getnf(ah, chan)) {
			/*
...
static bool ar9285_hw_cl_cal(struct ath_hw *ah, struct ath9k_channel *chan)
{
	...
	if (IS_CHAN_HT20(chan)) {
		...
		if (!ath9k_btcoex_ath9k_hw_wait(ah, AR_PHY_AGC_CONTROL,
	...
	REG_SET_BIT(ah, AR_PHY_AGC_CONTROL, AR_PHY_AGC_CONTROL_CAL);
	if (!ath9k_btcoex_ath9k_hw_wait(ah, AR_PHY_AGC_CONTROL, AR_PHY_AGC_CONTROL_CAL,
			  0, AH_WAIT_TIMEOUT)) {
		ath_dbg(common, CALIBRATE,
			"offset calibration failed to complete in 1ms; noisy environment?\n");
		return false;
