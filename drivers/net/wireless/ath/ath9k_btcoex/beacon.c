...
static void ath9k_beaconq_config(struct ath_softc *sc)
{
	...
	ath9k_btcoex_ath9k_hw_get_txq_props(ah, sc->beacon.beaconq, &qi);
	...
	if (sc->sc_ah->opmode == NL80211_IFTYPE_AP) {
		...
		ath9k_btcoex_ath9k_hw_get_txq_props(ah, txq->axq_qnum, &qi_be);
	...
	if (!ath9k_btcoex_ath9k_hw_set_txq_props(ah, sc->beacon.beaconq, &qi)) {
		ath_err(common, "Unable to update h/w beacon queue parameters\n");
	} else {
		ath9k_btcoex_ath9k_hw_resettxqueue(ah, sc->beacon.beaconq);
	}
}
...
static int ath9k_beacon_choose_slot(struct ath_softc *sc)
{
	...
	if (sc->sc_ah->opmode != NL80211_IFTYPE_AP) {
		ath_dbg(common, BEACON, "slot 0, tsf: %llu\n",
			ath9k_btcoex_ath9k_hw_gettsf64(sc->sc_ah));
		return 0;
	}
...
void ath9k_beacon_tasklet(unsigned long data)
{
	...
	if (ath9k_btcoex_ath9k_hw_numtxpending(ah, sc->beacon.beaconq) != 0) {
		...
		if (sc->beacon.bmisscnt < BSTUCK_THRESH * sc->nbcnvifs) {
			ath_dbg(common, BSTUCK,
				...
				sc->beacon.bmisscnt);
			ath9k_btcoex_ath9k_hw_stop_dma_queue(ah, sc->beacon.beaconq);
			if (sc->beacon.bmisscnt > 3)
				ath9k_btcoex_ath9k_hw_bstuck_nfcal(ah);
	...
	if (bf) {
		...
		ath9k_btcoex_ath9k_hw_puttxbuf(ah, sc->beacon.beaconq, bf->bf_daddr);

		if (!edma)
			ath9k_btcoex_ath9k_hw_txstart(ah, sc->beacon.beaconq);
	}
}
...
static void ath9k_beacon_init(struct ath_softc *sc, u32 nexttbtt, u32 intval)
{
	...
	ath9k_btcoex_ath9k_hw_reset_tsf(ah);
	ath9k_beaconq_config(sc);
	ath9k_btcoex_ath9k_hw_beaconinit(ah, nexttbtt, intval);
	sc->beacon.bmisscnt = 0;
				    ...
				    struct ath_beacon_config *conf)
{
	...
	ath9k_btcoex_ath9k_hw_disable_interrupts(ah);
	ath9k_btcoex_ath9k_hw_set_sta_beacon_timers(ah, &bs);
