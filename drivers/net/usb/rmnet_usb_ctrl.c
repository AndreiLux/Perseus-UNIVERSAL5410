...
static void ctrl_write_callback(struct urb *urb)
{
	...
	if (urb->status) {
		dev->tx_ctrl_err_cnt++;
		pr_err_ratelimited("Write status/size %d/%d\n",
				urb->status, urb->actual_length);
#ifdef CONFIG_MDM_HSIC_PM
		if(dev->tx_ctrl_err_cnt > 0) {
...
ctrl_read:
	...
	DBG("%s: Returning %d bytes to %s\n", __func__, bytes_to_read,
			dev->name);
	DUMP_BUFFER("mdm: <<[RX]: ", bytes_to_read, buf);
...
retry_submit:
	...
	DUMP_BUFFER("mdm: [TX]>>: ", urb->transfer_buffer_length,
							urb->transfer_buffer);
