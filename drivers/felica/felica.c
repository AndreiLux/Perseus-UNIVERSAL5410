...
static irqreturn_t intu_poll_irq_handler(int irq, void *dev_id)
{
	...
	//HW-REV-0.3(0x01)
	disable_irq_nosync(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03));
...
static void intu_poll_work_func(struct work_struct *work)
{
	...
	old_value = intu_d->device_status;
		read_value = gpio_get_value(GPIO_PINID_NFC_INTU_REV03);
	...
	if (read_value < 0) {
		...
		if (intu_d->device_status == INTU_POLL_DEV_LOW){
			...
			if (irq_set_irq_type(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03), 
				irqflag)){
	...
	enable_irq(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03));
...
static int intu_poll_init(void)
{
	...
	//HW-REV-0.3(0x01)
	ret = gpio_get_value(GPIO_PINID_NFC_INTU_REV03);
	if (ret < 0) {
		g_intu_d->open_flag = 0;
		FELICA_LOG_ERR("[MFDD] %s ERROR(gpio_get_value), ret=[%d]",
						__func__, ret);
	...
	if (request_irq(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03), 
	...
	if (enable_irq_wake(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03))){
		
		FELICA_LOG_ERR("[MFDD] %s ERROR(enable_irq_wake)", __func__);
		
		free_irq(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03), (void *)g_intu_d);
...
static void intu_poll_exit(void)
{
	...
	//HW-REV-0.3(0x01)
	if (disable_irq_wake(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03))){
		FELICA_LOG_ERR("[MFDD] %s ERROR(disable_irq_wake)", __func__);
	}
	free_irq(gpio_to_irq(GPIO_PINID_NFC_INTU_REV03), (void *)intu_d);
