...
int init_table_info(struct str_smart_dim *smart)
{
	...
	smart->gamma_table[G_19] = GAMMA_CONTROL_TABLE[G_19];
	smart->gamma_table[G_20] = GAMMA_CONTROL_TABLE[G_20];
...
u32 calc_gamma_table(struct str_smart_dim *smart, struct aid_info *aid, u8 result[], const u8 *mtp, u8 array_ct)
{
	u32 i, c, t1;
	u32 temp;
	u32 lidx;
