...
static u32 calc_v3_volt(s16 gamma, int rgb_index, u32 adjust_volt[CI_MAX][AD_IVMAX])
{
	...
	ratio = volt_table_cv_64_dv_320_6P[gamma];
...
static u32 calc_v255_volt(s16 gamma, int rgb_index, u32 adjust_volt[CI_MAX][AD_IVMAX])
{
	...
	ret = volt_table_v255_6P[gamma] >> 16;

	return ret;
}

u8 calc_voltage_table_6P(struct str_smart_dim *smart, const u8 *mtp)
{
	int c, i, j;
...
int init_table_info_6P(struct str_smart_dim *smart)
{
	int i;
	int offset = 0;
	...
	smart->flooktbl = flookup_table_6P;
	smart->g300_gra_tbl = gamma_300_gra_table_6P;
	smart->gamma_table[G_160] = GAMMA_CONTROL_TABLE_6P[G_160];
	smart->gamma_table[G_165] = GAMMA_CONTROL_TABLE_6P[G_165];
	smart->gamma_table[G_170] = GAMMA_CONTROL_TABLE_6P[G_170];
	smart->gamma_table[G_175] = GAMMA_CONTROL_TABLE_6P[G_175];
	smart->gamma_table[G_180] = GAMMA_CONTROL_TABLE_6P[G_180];
	smart->gamma_table[G_185] = GAMMA_CONTROL_TABLE_6P[G_185];
	smart->gamma_table[G_190] = GAMMA_CONTROL_TABLE_6P[G_190];
...
u32 calc_gamma_table_6P(struct str_smart_dim *smart, struct aid_info *aid, u8 result[], const u8 *mtp, u8 array_ct)
{
	u32 i, c, t1;
	u32 temp;
	u32 lidx;
	...
	for (i = IV_3 ; i < IV_MAX; i++) {
		if (i ==  IV_3) {
			...
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V3];
		} else if (i ==  IV_11) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V11];
		} else if (i ==  IV_23) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V23];
		} else if (i ==  IV_35) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V35];
		} else if (i ==  IV_51) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V51];
		} else if (i ==  IV_87) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V87];
		} else if (i ==  IV_151) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V151];
		} else if (i ==  IV_203) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V203];
		} else if (i ==  IV_255) {
			temp = (smart->gamma_table[(aid+array_ct)->gamma][dv_value[i]] * (aid+array_ct)->base_cd)/1000;
			lidx = lookup_vtbl_idx(smart, temp) + (aid+array_ct)->offsets[V255];
	}
