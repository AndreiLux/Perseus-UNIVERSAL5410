...
#include "s6e3fa0_rgb_param.h"

#include "dynamic_aid_s6e3fa0_rgb.h"
...
static int s6e3fa0_set_elvss(struct lcd_info *lcd, u8 force)
{
	...
	if (force || lcd->elvss_table[lcd->acl_enable][lcd->current_elvss][2] != elvss) {
		if (lcd->temperature == TSET_25_DEGREES)
...
static int init_elvss_table(struct lcd_info *lcd, u8 *elvss_data)
{
	...
	for (k = 0; k < 2; k++) {
		...
		for (i = 0; i < ELVSS_STATUS_MAX; i++) {
			...
			lcd->elvss_table[k][i][2] += ELVSS_TABLE[i][k];
		}
...
static int init_tset_table(struct lcd_info *lcd)
{
	...
	for (i = 0; i < TSET_STATUS_MAX; i++) {
		...
		for (j = 0; j < ARRAY_SIZE(SEQ_SWIRE_CONTROL); j++)
			lcd->tset_table[i][j] = SEQ_SWIRE_CONTROL[j];
	...
	const u8 *mtp_data, const u8 *hbm_data, const u8 *elvss_data)
{
	...
	lcd->elvss_table[0][ELVSS_STATUS_HBM][2] = 0x0A;
	lcd->elvss_table[1][ELVSS_STATUS_HBM][2] = 0x0A;

	lcd->elvss_hbm[1] = mtp_data[LDI_HBM_LEN - 1];
...
static void s6e3fa0_ldi_touchkey_off(void)
{
	...
	if (lcd->ldi_enable) {
		s6e3fa0_write(lcd, SEQ_TEST_KEY_ON_FC, ARRAY_SIZE(SEQ_TEST_KEY_ON_FC));
		s6e3fa0_write(lcd, SEQ_TOUCHKEY_OFF, ARRAY_SIZE(SEQ_TOUCHKEY_OFF));
		s6e3fa0_write(lcd, SEQ_TEST_KEY_OFF_FC, ARRAY_SIZE(SEQ_TEST_KEY_OFF_FC));
	}
}
...
static int s6e3fa0_ldi_init(struct lcd_info *lcd)
{
	...
	if (lcd->id[2] < 0x20) /* OP Manual: ~ revE */
	...
	if (lcd->id[2] < 0x20) /* OP Manual: ~ revE */
		s6e3fa0_write(lcd, SEQ_RE_SETTING_EVT0, ARRAY_SIZE(SEQ_RE_SETTING_EVT0));
	else {
	...
	/* 3. ELVSS Control */
	s6e3fa0_write(lcd, SEQ_SWIRE_CONTROL, ARRAY_SIZE(SEQ_SWIRE_CONTROL));
	s6e3fa0_write(lcd, SEQ_ELVSS_CONDITION_SET, ARRAY_SIZE(SEQ_ELVSS_CONDITION_SET));
#endif
	...
	if (lcd->id[2] < 0x20) /* OP Manual: ~ revE */
		...
		s6e3fa0_write(lcd, SEQ_ERR_FG_EVT1, ARRAY_SIZE(SEQ_ERR_FG));
	...
	struct device_attribute *attr, char *buf)
{
	...
	pos += sprintf(pos, "ELVSS [B6]: %02x, %02x, %02x \n",
		temp[0], temp[1], temp[2] );
