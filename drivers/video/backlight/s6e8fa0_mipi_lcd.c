...
static int s6e8fa0_ldi_init(struct lcd_info *lcd)
{
	...
	s6e8fa0_write(lcd, SEQ_TEST_KEY_ON_F1, ARRAY_SIZE(SEQ_TEST_KEY_ON_F1));
	s6e8fa0_write(lcd, SEQ_TEST_KEY_ON_FC, ARRAY_SIZE(SEQ_TEST_KEY_ON_FC));
	s6e8fa0_write(lcd, SEQ_SCAN_TIMMING_1_1, ARRAY_SIZE(SEQ_SCAN_TIMMING_1_1));
	s6e8fa0_write(lcd, SEQ_SCAN_TIMMING_1_2, ARRAY_SIZE(SEQ_SCAN_TIMMING_1_2));
	s6e8fa0_write(lcd, SEQ_SCAN_TIMMING_1_3, ARRAY_SIZE(SEQ_SCAN_TIMMING_1_3));
	s6e8fa0_write(lcd, SEQ_SLEEP_OUT, ARRAY_SIZE(SEQ_SLEEP_OUT));
	s6e8fa0_write(lcd, SEQ_ERROR_FLAG_ON, ARRAY_SIZE(SEQ_ERROR_FLAG_ON));

	msleep(130);

	s6e8fa0_write(lcd, SEQ_LTPS_PANEL_CONTROL, ARRAY_SIZE(SEQ_LTPS_PANEL_CONTROL));
	s6e8fa0_write(lcd, SEQ_PANEL_UPDATE, ARRAY_SIZE(SEQ_PANEL_UPDATE));
	s6e8fa0_write(lcd, SEQ_SCAN_TIMMING_2_1, ARRAY_SIZE(SEQ_SCAN_TIMMING_2_1));
	s6e8fa0_write(lcd, SEQ_SCAN_TIMMING_2_2, ARRAY_SIZE(SEQ_SCAN_TIMMING_2_2));
	s6e8fa0_write(lcd, SEQ_BRIGHTNESS_CONDITION_SET, ARRAY_SIZE(SEQ_BRIGHTNESS_CONDITION_SET));
	s6e8fa0_write(lcd, SEQ_GAMMA_UPDATE, ARRAY_SIZE(SEQ_GAMMA_UPDATE));
	s6e8fa0_write(lcd, SEQ_BRIGHTNESS_CONTROL_1, ARRAY_SIZE(SEQ_BRIGHTNESS_CONTROL_1));
	s6e8fa0_write(lcd, SEQ_BRIGHTNESS_CONTROL_2, ARRAY_SIZE(SEQ_BRIGHTNESS_CONTROL_2));
...
static int s6e8fa0_fhd_set_power(struct lcd_device *ld, int power)
{
...
static int s6e8fa0_fhd_get_power(struct lcd_device *ld)
{
	struct lcd_info *lcd = lcd_get_data(ld);
...
static int s6e8fa0_fhd_set_brightness(struct backlight_device *bd)
{
	int ret = 0;
...
static int s6e8fa0_fhd_get_brightness(struct backlight_device *bd)
{
	struct lcd_info *lcd = bl_get_data(bd);
...
static int s6e8fa0_fhd_check_fb(struct lcd_device *ld, struct fb_info *fb)
{
...
static struct lcd_ops s6e8fa0_fhd_lcd_ops = {
	.set_power = s6e8fa0_fhd_set_power,
	.get_power = s6e8fa0_fhd_get_power,
	.check_fb  = s6e8fa0_fhd_check_fb,
};

static const struct backlight_ops s6e8fa0_fhd_backlight_ops  = {
	.get_brightness = s6e8fa0_fhd_get_brightness,
	.update_status = s6e8fa0_fhd_set_brightness,
};
...
static int s6e8fa0_probe(struct mipi_dsim_device *dsim)
{
	...
	lcd->ld = lcd_device_register("panel", dsim->dev, lcd, &s6e8fa0_fhd_lcd_ops);
	if (IS_ERR(lcd->ld)) {
	...
	lcd->bd = backlight_device_register("panel", dsim->dev, lcd, &s6e8fa0_fhd_backlight_ops, NULL);
	if (IS_ERR(lcd->bd)) {
...
static int s6e8fa0_fhd_init(void)
{
	return 0 ;
#if 0
...
static void s6e8fa0_fhd_exit(void)
{
	return;
}

module_init(s6e8fa0_fhd_init);
module_exit(s6e8fa0_fhd_exit);
