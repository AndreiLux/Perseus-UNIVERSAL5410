...
static char zero_cfg_buf[] = {29, 28, 59, 126, 0, 120, 240, 126, 124, 147, 73,
	126, 28, 149, 42, 126, 12, 20, 58, 126, 99, 229, 161, 126, 75, 15, 0,
	0, 187, 96, 126, 75, 9, 0, 0, 98, 182, 126, 75, 8, 0, 0, 190, 236, 126,
	75, 8, 1, 0, 102, 245, 126, 75, 4, 0, 0, 29, 73, 126, 75, 4, 15, 0,
	213, 202, 126, 125, 93, 5, 0, 0, 0, 0, 0, 0, 116, 65, 126, 115, 0, 0,
	0, 0, 0, 0, 0, 218, 129, 126, 96, 0, 18, 106, 126};
/* zero cfg packet is divided into number of sub packets
   and the size of each sub packet is given below*/
static unsigned int zero_cfg_packet_lens[] = {4, 4, 4, 4, 4, 4,
	7, 7, 7, 7, 7, 7, 12, 11, 5};
		...
		int buf_size, int actual_size)
{
	...
	if (driver->zero_cfg_mode) {
		...
		driver->zero_cfg_packet_lens_index++;
		if (driver->zero_cfg_packet_lens_index ==
				ZERO_CFG_SUBPACKET_MAX) {
			...
			pr_info("%s zero_cfg sub packet number:%d\n", __func__,
					driver->zero_cfg_packet_lens_index);
			queue_work(driver->diag_hsic_wq,
					&driver->diag_zero_cfg_hsic_work);
		}
		return ;
	}
...
int diag_hsic_enable(void)
{
	...
	INIT_WORK(&(driver->diag_zero_cfg_hsic_work),
			diag_zero_cfg_hsic_work_fn);
