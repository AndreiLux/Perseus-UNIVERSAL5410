		...
		if ((atomic_read(&data->aSensorEnable) & (1 << uChangedSensor))
			...
			if (!(uNewEnable & (1 << uChangedSensor)))
				ssp_remove_sensor(data, uChangedSensor,
					uNewEnable); /* disable */
			/* In case of enabling */
			/* we will send instruction on change_sensor_delay. */
			else if (!((uNewEnable & (1 << uChangedSensor))
						& data->uSensorState)) {
...
static ssize_t show_uncal_gyro_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);

	return sprintf(buf, "%lld\n", data->adDelayBuf[UNCALIBRATED_GYRO]);
}

static ssize_t set_uncal_gyro_delay(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int64_t dNewDelay;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	change_sensor_delay(data, UNCALIBRATED_GYRO, dNewDelay);
...
static ssize_t show_grot_vec_delay(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);
...
static ssize_t set_grot_vec_delay(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int64_t dNewDelay;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%lld\n",
		data->adDelayBuf[SIGNIFICANT_MOTION]);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	change_sensor_delay(data, SIGNIFICANT_MOTION, dNewDelay);
	return size;
}
...
static struct device_attribute dev_attr_uncal_gyro_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_uncal_gyro_delay, set_uncal_gyro_delay);
static struct device_attribute dev_attr_grot_vec_poll_delay
	= __ATTR(poll_delay, S_IRUGO | S_IWUSR | S_IWGRP,
	show_grot_vec_delay, set_grot_vec_delay);
...
int initialize_sysfs(struct ssp_data *data)
{
	...
	if (device_create_file(&data->uncal_gyro_input_dev->dev,
		&dev_attr_uncal_gyro_poll_delay))
		goto err_uncal_gyro_input_dev;

	if (device_create_file(&data->grot_vec_input_dev->dev,
		&dev_attr_grot_vec_poll_delay))
		goto err_grot_vec_input_dev;
...
err_grot_vec_input_dev:
	device_remove_file(&data->uncal_gyro_input_dev->dev,
		&dev_attr_uncal_gyro_poll_delay);
err_uncal_gyro_input_dev:
...
void remove_sysfs(struct ssp_data *data)
{
	...
	device_remove_file(&data->uncal_gyro_input_dev->dev,
		&dev_attr_uncal_gyro_poll_delay);
	device_remove_file(&data->grot_vec_input_dev->dev,
		&dev_attr_grot_vec_poll_delay);
