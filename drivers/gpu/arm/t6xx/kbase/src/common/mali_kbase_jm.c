...
mali_error kbase_job_slot_init(kbase_device *kbdev)
{
	...
	osk_err = osk_spinlock_irq_init(&kbdev->jm_slots_lock, OSK_LOCK_ORDER_JSLOT);
...
void kbase_job_slot_term(kbase_device *kbdev)
{
	...
	osk_spinlock_irq_term(&kbdev->jm_slots_lock);
}
...
void kbase_job_slot_hardstop(kbase_context *kctx, int js, kbase_jd_atom *target_katom)
{
	    ...
	    kbase_hw_has_issue(kctx->kbdev, BASE_HW_ISSUE_9510))
	{
		 ...
		 */
		osk_spinlock_irq_unlock(&kbdev->jm_slots_lock); /* UNLOCK instead of lock */
		if (kbase_prepare_to_reset_gpu(kbdev))
		{
			kbase_reset_gpu(kbdev);
		}
		osk_spinlock_irq_lock(&kbdev->jm_slots_lock); /* LOCK instead of unlock */
	}
}
