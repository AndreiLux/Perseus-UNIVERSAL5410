...
static struct alc5625_init_reg alc5625_init_list[] = {
	...
	{ALC5625_HP_OUT_VOL,		0x8888}, /* default is -12db */
...
static int set_vodsp_aec_path(struct snd_soc_codec *codec, unsigned int mode)
{
	...
	case PCM_IN_PCM_OUT:
		...
		/* set AEC path */
		alc5625_write_mask(codec, 0x26, 0x0300, 0x0300);
		...
		alc5625_write_mask(codec, ALC5625_VODSP_CTL,
					VODSP_LRCK_SEL_8K,
					VODSP_LRCK_SEL_MASK);
		alc5625_write_mask(codec, 0x26, 0x0000, 0x0300);
		 ...
		 */
		alc5625_write_mask(codec, 0x3a, 0x0c8f, 0x0c8f);
		 ...
		 */
		alc5625_write_mask(codec, 0x3c, 0xa4cb, 0xa4cb);

		/* power spk left/right vol | pwr vodsp interface |
		 * power on microphone1 boost
		 */
		alc5625_write_mask(codec, 0x3e, 0x3302, 0xf302);

		/* Mute DAC to hpmixer */
		alc5625_write(codec, 0x10, 0xee0f);

		/* Set Mic1 to differential mode */
		alc5625_write(codec, 0x0e, 0x8808);

		/* Mic boost 0db */
		alc5625_write(codec, 0x22, 0x0000);

		/* ADC_Mixer_R boost 10.5 db */
		alc5625_write(codec, 0x12, 0xcbd3);

		/* Mic1->ADCMixer_R */
		alc5625_write(codec, 0x14, 0x7f3f);

		/* VoDAC to speakerMixer,0db */
		alc5625_write(codec, 0x18, 0xa010);

		/* Speaker source from speakermixer */
		alc5625_write(codec, 0x1c, 0x8808);

		/* Unmute speaker */
		alc5625_write_mask(codec, 0x02, 0x0000, 0x8080);
	...
	case ANALOG_IN_ANALOG_OUT:
		...
		/* Set input&output path and power */
		/* Power on related bit */
		alc5625_write_mask(codec, 0x3a, 0xcc8f, 0xcc8f);

		/* Power on related bit */
		alc5625_write_mask(codec, 0x3c, 0xa7cf, 0xa7cf);

		/* Power on related bit */
		alc5625_write_mask(codec, 0x3e, 0xf312, 0xf312);

		/* Set Mic1 to differential mode */
		alc5625_write(codec, 0x0e, 0x8808);

		/* Set phone in to differential mode */
		alc5625_write(codec, 0x08, 0xe800);

		/* Mic boost 0db */
		alc5625_write(codec, 0x22, 0x0000);

		/* Mic1->ADCMixer_R,phone in-->ADCMixer_L */
		alc5625_write(codec, 0x14, 0x773f);

		/* ADC_Mixer_R boost 10.5 db */
		alc5625_write(codec, 0x12, 0xCBD3);

		/* Speaker from spkmixer,monoOut from monoMixer */
		alc5625_write(codec, 0x1c, 0x88c8);

		/* Unmute VoDAC to spkmixer */
		alc5625_write(codec, 0x18, 0xA010);

		/* Unmute DAC to monoMixer */
		alc5625_write(codec, 0x10, 0xee0e);
		alc5625_write(codec, 0x62, 0x2222);
		alc5625_write(codec, 0x64, 0x3122);

		/* Unmute speaker */
		alc5625_write_mask(codec, 0x02, 0x0000, 0x8080);

		/* Unmute auxout */
		alc5625_write_mask(codec, 0x06, 0x0000, 0x8080);
		break;
	...
	case DAC_IN_ADC_OUT:
		...
		/* Set input&output path and power */
		/* Power on related bit */
		alc5625_write_mask(codec, 0x3a, 0xcc0f, 0xcc0f);

		/* Power on related bit */
		alc5625_write_mask(codec, 0x3c, 0xa7cb, 0xa7cb);

		/* Power on related bit */
		alc5625_write_mask(codec, 0x3e, 0x3302, 0x3302);
		...
		alc5625_write(codec, 0x10, 0xee0d);
		alc5625_write(codec, 0x60, 0x3075);
		alc5625_write(codec, 0x62, 0x1010);
	...
	default:
		/* Mute speaker out */
		alc5625_write_mask(codec, 0x02, 0x8080, 0x8080);

		/* Mute auxout */
		alc5625_write_mask(codec, 0x06, 0x8080, 0x8080);

		/* Mic boost 20db by default */
		alc5625_write(codec, 0x22, 0x0500);

		/* Record from Mic1 by default */
		alc5625_write(codec, 0x14, 0x3f3f);

		/* ADC_Mixer_R boost 15 db by default */
		alc5625_write(codec, 0x12, 0xD5D5);

		/* All output from HPmixer by default */
		alc5625_write(codec, 0x1c, 0x0748);

		/* DAC to HPmixer by default */
		alc5625_write(codec, 0x10, 0xee03);

		/* Mute VoDAC to mixer by default */
		alc5625_write(codec, 0x18, 0xe010);
		...
		alc5625_write_mask(codec, 0x3e, 0x0000, 0xf312);

		/* Power down related bit */
		alc5625_write_mask(codec, 0x3a, 0x0000, 0xcc8d);

		/* Power down related bit */
		alc5625_write_mask(codec, 0x3c, 0x0000, 0x07cf);
		break;
	}
	return 0;
}
					...
					struct snd_soc_dai *dai)
{
	...
	alc5625_write_mask(codec, 0x3a, 0xc801, 0xc801);
	if (coeff >= 0) {
		...
		struct snd_soc_dai *dai)
{
	...
	/* power i2s and dac ref */
	alc5625_write_mask(codec, 0x3a, 0x0801, 0x0801);
		...
		enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		alc5625_write(codec, 0x26, 0x0000);
		alc5625_write_mask(codec, 0x3c, 0x2000, 0x2000);
		alc5625_write_mask(codec, 0x3a, 0x000e, 0x000e);
		break;
	case SND_SOC_BIAS_STANDBY:
		break;
	case SND_SOC_BIAS_OFF:
		alc5625_write_mask(codec, 0x04, 0x8080, 0x8080); /* mute hp */
		alc5625_write_mask(codec, 0x02, 0x8080, 0x8080); /* mute spk */
		alc5625_write(codec, 0x3e, 0x0000); /* power off all bit */
		alc5625_write(codec, 0x3a, 0x0000); /* power off all bit */
		alc5625_write(codec, 0x3c, 0x0000); /* power off all bit */
		break;
	}
