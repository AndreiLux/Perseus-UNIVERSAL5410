...
#ifdef CONFIG_MDM_HSIC_PM
		/* write blocked till mdm ready to data receive
		 * when mdm send serial state of sth on this channel
		 * it means it can rx data over hsic, so clear lpa flag here
		 */
		pr_info("mdm: Ch%d, clear lpa state\n", iface_num);
		lpa_handling = false;
#endif
		break;
	default:
		...
		int id)
{
	...
	if (retval < 0) {
		dev_err(&dev->udev->dev,
			"%s: unable to add platform device\n", __func__);
		goto free_in_ctlreq;
	}
