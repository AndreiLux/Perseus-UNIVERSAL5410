...
static void read_hsic(void)
{
	...
	pr_debug("mdm: diag, start handling incoming useless packets\n");
	if (!dev->ifc) {
...
static void read_hsic_cb(struct urb *urb)
{
	...
	case -ENOENT:
		if (urb->actual_length)
			pr_info("mdm: diag, drop useless %d bytes\n",
							urb->actual_length);
		break;
	case -EPROTO:
	...
	default:
		/* do nothing */
		pr_err("%s: drop packet from err %d\n", __func__, urb->status);
	}
}
...
diag_bridge_probe(struct usb_interface *ifc, const struct usb_device_id *id)
{
	...
	pr_info("%s: id:%lu", __func__, id->driver_info);
	...
	if (ret < 0) {
		err("%s: unable to add platform device\n", __func__);
		goto error;
	}
