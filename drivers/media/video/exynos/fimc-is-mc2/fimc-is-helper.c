...
int fimc_is_hw_wait_intmsr0_intmsd0(struct fimc_is_core *dev)
{
	...
	while (status) {
		dbg("[MAIN] INTMSR1's 0 bit is not cleared.\n");
...
void fimc_is_hw_open_sensor(struct fimc_is_core *dev, u32 id, u32 sensor_index)
{
	...
	writel(sensor_index, dev->regs + ISSR2);
	writel(sensor->enum_sensor[sensor_index].i2c_ch, dev->regs + ISSR3);
...
void fimc_is_hw_close_sensor(struct fimc_is_core *dev, u32 id)
{
...
int fimc_is_hw_a5_power_on(struct fimc_is_core *isp)
{
	u32 cfg;
	u32 timeout;
	...
	if (isp->pdata->sensor_power_on) {
		isp->pdata->sensor_power_on(isp->pdev, 0);
	} else {
...
int fimc_is_hw_a5_power_off(struct fimc_is_core *isp)
{
	u32 timeout;
...
void fimc_is_hw_a5_power(struct fimc_is_core *isp, int on)
{
...
void fimc_is_hw_set_init(struct fimc_is_core *dev)
{
	...
	IS_ISP_SET_PARAM_AA_FACE(dev,
		init_val_isp_preview_still.aa.uiAfFace);
...
void fimc_is_hw_set_default_size(struct fimc_is_core *dev, int  sensor_id)
{
	...
	case SENSOR_NAME_S5K6A3:
		dev->video_scc.common.frame.width =
			DEFAULT_6A3_STILLSHOT_WIDTH;
		dev->video_scc.common.frame.height =
			DEFAULT_6A3_STILLSHOT_HEIGHT;
		dev->video_scp.common.frame.width =
			DEFAULT_6A3_PREVIEW_WIDTH;
		dev->video_scp.common.frame.height =
