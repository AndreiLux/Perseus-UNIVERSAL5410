			...
			void *allocators[])
{
	...
	*num_planes = 1;
	...
	sizes[0] = DEFAULT_CAPTURE_STILL_WIDTH*DEFAULT_CAPTURE_STILL_HEIGHT*2;
...
static int fimc_is_scalerp_start_streaming(struct vb2_queue *q, unsigned int count)
{
	struct fimc_is_video_dev *video = q->drv_priv;
	struct fimc_is_dev	*isp = video->dev;
	int ret;
...
static int fimc_is_3dnr_start_streaming(struct vb2_queue *q, unsigned int count)
{
	struct fimc_is_video_dev *video = q->drv_priv;
	struct fimc_is_dev	*isp = video->dev;
	int ret;
