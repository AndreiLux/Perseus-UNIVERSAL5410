...
static int tdmb_ant_det_check_value(void)
{
	...
	for (loop = 0; loop < TDMB_ANT_CHECK_COUNT; loop++) {
		...
		cur_val = gpio_get_value_cansleep(GPIO_TDMB_ANT_DET);
...
static bool tdmb_ant_det_reg_input(struct platform_device *pdev)
{
	...
	ant_prev_status = gpio_get_value_cansleep(GPIO_TDMB_ANT_DET);
...
static bool tdmb_ant_det_irq_set(bool set)
{
	...
	if (set) {
			irq_set_irq_type(GPIO_TDMB_IRQ_ANT_DET
					, IRQ_TYPE_EDGE_BOTH);

		irq_ret = request_irq(GPIO_TDMB_IRQ_ANT_DET
		...
		enable_irq_wake(GPIO_TDMB_IRQ_ANT_DET);
	} else {
		disable_irq_wake(GPIO_TDMB_IRQ_ANT_DET);
		free_irq(GPIO_TDMB_IRQ_ANT_DET, NULL);
	}
