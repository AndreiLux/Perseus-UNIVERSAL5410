...
static struct qc_dpram_boot_map qc_bt_map[MAX_MODEM_TYPE];
static struct qc_dpram_udl_param qc_udl_param[MAX_MODEM_TYPE];
static struct qc_dpram_udl_check qc_udl_check[MAX_MODEM_TYPE];

static void qc_dload_task(unsigned long data);

static void qc_init_boot_map(struct pld_link_device *pld)
{
	struct qc_dpram_boot_map *qbt_map = &qc_bt_map[pld->ld.mdm_data->modem_type];
...
static int qc_prepare_download(struct pld_link_device *pld)
{
	...
	while (1) {
		if (qc_udl_check[pld->ld.mdm_data->modem_type].copy_start) {
			qc_udl_check[pld->ld.mdm_data->modem_type].copy_start = 0;
			break;
		}
			...
			enum qc_dload_tag tag)
{
	...
	unsigned char *img;
	struct qc_dpram_udl_param param;
	int modem_type = pld->ld.mdm_data->modem_type;
	...
	qc_udl_check[modem_type].total_size = param.size;
	qc_udl_check[modem_type].rest_size = param.size;
	qc_udl_check[modem_type].send_size = 0;
	qc_udl_check[modem_type].copy_complete = 0;

	qc_udl_param[modem_type].addr = img;
	qc_udl_param[modem_type].size = pld->dpram->max_boot_frame_size;
	if (tag == QC_DLOAD_TAG_NV)
		qc_udl_param[modem_type].count = 1;
	else
		qc_udl_param[modem_type].count = param.count;
	qc_udl_param[modem_type].tag = tag;

	if (qc_udl_check[modem_type].rest_size < pld->dpram->max_boot_frame_size)
		qc_udl_param[modem_type].size = qc_udl_check[modem_type].rest_size;

	/* Download image (binary or NV) */
	_qc_do_download(pld, &qc_udl_param[modem_type]);
	...
	*/
	...
	while (1) {
		if (qc_udl_check[modem_type].copy_complete) {
			qc_udl_check[modem_type].copy_complete = 0;
			retval = 0;
			break;
		}
...
static void qc_dload_task(unsigned long data)
{
	...
	qc_udl_check[modem_type].send_size += qc_udl_param[modem_type].size;
	qc_udl_check[modem_type].rest_size -= qc_udl_param[modem_type].size;

	qc_udl_param[modem_type].addr += qc_udl_param[modem_type].size;

	if (qc_udl_check[modem_type].send_size >= qc_udl_check[modem_type].total_size) {
		qc_udl_check[modem_type].copy_complete = 1;
		qc_udl_param[modem_type].tag = 0;
		return;
	}
...
static void qc_dload_cmd_handler(struct pld_link_device *pld, u16 cmd)
{
	...
	case 0x1234:
		mif_info("[%s] cmd 0x%x\n", pld->ld.name, cmd);
		qc_udl_check[pld->ld.mdm_data->modem_type].copy_start = 1;
		break;
	...
	case 0xABCD:
		...
		qc_udl_check[pld->ld.mdm_data->modem_type].boot_complete = 1;
		break;
...
static int qc_boot_start(struct pld_link_device *pld)
{
	...
	while (1) {
		if (qc_udl_check[pld->ld.mdm_data->modem_type].boot_complete) {
			qc_udl_check[pld->ld.mdm_data->modem_type].boot_complete = 0;
			break;
		}
...
static void qc_crash_log(struct pld_link_device *pld)
{
	...
	iowrite16(PLD_ADDR_MASK(data), pld->address_buffer);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DPRAM_PHONEIMG_LOAD:
		mif_info("%s: downloading phone image.\n", ld->name);
