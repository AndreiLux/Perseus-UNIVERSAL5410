...
static void cbp82_init_ipc_map(struct dpram_link_device *dpld)
{
	struct dpram_ipc_16k_map *map;
	struct dpram_ipc_device *dev;
	mif_err("+++\n");

	map = (struct dpram_ipc_16k_map *)dpld->base;

	/* Magic code and access enable fields */
	dpld->ipc_map.magic = (u16 __iomem *)&map->magic;
	dpld->ipc_map.access = (u16 __iomem *)&map->access;
	...
	dev->txq.head = (u16 __iomem *)&map->fmt_tx_head;
	dev->txq.tail = (u16 __iomem *)&map->fmt_tx_tail;
	dev->txq.buff = (u8 __iomem *)&map->fmt_tx_buff[0];
	dev->txq.size = DP_16K_FMT_TX_BUFF_SZ;

	dev->rxq.head = (u16 __iomem *)&map->fmt_rx_head;
	dev->rxq.tail = (u16 __iomem *)&map->fmt_rx_tail;
	...
	dev->txq.head = (u16 __iomem *)&map->raw_tx_head;
	dev->txq.tail = (u16 __iomem *)&map->raw_tx_tail;
	dev->txq.buff = (u8 __iomem *)&map->raw_tx_buff[0];
	dev->txq.size = DP_16K_RAW_TX_BUFF_SZ;

	dev->rxq.head = (u16 __iomem *)&map->raw_rx_head;
	dev->rxq.tail = (u16 __iomem *)&map->raw_rx_tail;
	...
	/* Interrupt ports */
	dpld->ipc_map.mbx_cp2ap = (u16 __iomem *)&map->mbx_ap2cp;
	dpld->ipc_map.mbx_ap2cp = (u16 __iomem *)&map->mbx_cp2ap;
...
static struct dpram_ext_op ext_op_set[MAX_MODEM_TYPE] = {
	...
	[VIA_CBP82] = {
		.exist = 1,
		.init_ipc_map = cbp82_init_ipc_map,
	},
#endif
