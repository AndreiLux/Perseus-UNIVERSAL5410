...
static inline u16 recv_intr(struct pld_link_device *pld)
{
	...
	do {
		...
		mif_err("%s: ERR: val1(%x) != val2(%x)\n",
			((struct link_device *)&pld->ld)->name, val1, val2);
...
static inline void send_intr(struct pld_link_device *pld, u16 mask)
{
	...
	do {
		...
		mif_err("%s: ERR: intr1(%d) != intr2(%d)\n",
			((struct link_device *)&pld->ld)->name, val, mask);
		udelay(100);
...
static inline void set_magic(struct pld_link_device *pld, u16 in)
{
	...
	do {
		...
		mif_err("%s: ERR: magic1(%d) != magic2(%d)\n",
			((struct link_device *)&pld->ld)->name, val, in);
		udelay(100);
...
static inline u16 get_access(struct pld_link_device *pld)
{
	...
	do {
		...
		mif_err("%s: ERR: access1(%d) != access2(%d)\n",
			((struct link_device *)&pld->ld)->name, val1, val2);
		udelay(100);
...
static inline void set_access(struct pld_link_device *pld, u16 in)
{
	...
	do {
		...
		mif_err("%s: ERR: access(%d) != access(%d)\n",
			((struct link_device *)&pld->ld)->name, val, in);
		udelay(100);
...
static inline u32 get_tx_head(struct pld_link_device *pld, int id)
{
	...
	do {
		...
		mif_err("%s: ERR: %s txq.head(%d) != in(%d)\n",
			((struct link_device *)&pld->ld)->name,
...
static inline u32 get_tx_tail(struct pld_link_device *pld, int id)
{
	...
	do {
		...
		mif_err("%s: ERR: %s txq.tail(%d) != in(%d)\n",
			((struct link_device *)&pld->ld)->name,
				get_dev_name(id), val1, val2);
		udelay(100);
...
static inline u32 get_rx_head(struct pld_link_device *pld, int id)
{
	...
	do {
		...
		mif_err("%s: ERR: %s rxq.head(%d) != in(%d)\n",
			((struct link_device *)&pld->ld)->name,
				get_dev_name(id), val1, val2);
		udelay(100);
...
static inline u32 get_rx_tail(struct pld_link_device *pld, int id)
{
	...
	do {
		...
		mif_err("%s: ERR: %s rxq.tail(%d) != in(%d)\n",
			((struct link_device *)&pld->ld)->name,
				get_dev_name(id), val1, val2);
		udelay(100);
...
static inline void set_rx_tail(struct pld_link_device *pld, int id, u32 out)
{
	...
	do {
		...
		mif_err("%s: ERR: %s rxq.tail(%d) != out(%d)\n",
			((struct link_device *)&pld->ld)->name,
		...
		u32 *in, u32 *out)
{
	...
	do {
		...
		mif_info("%s: CAUTION! <%pf> %s_TXQ invalid (size:%d in:%d out:%d)\n",
		...
		u32 *in, u32 *out)
{
	...
	do {
		...
		mif_info("%s: CAUTION! <%pf> %s_RXQ invalid (size:%d in:%d out:%d)\n",
			ld->name, __builtin_return_address(0),
			get_dev_name(dev), qsize, head, tail);
...
static int pld_try_ipc_tx(struct pld_link_device *pld, int dev)
{
	...
	while (1) {
		...
		if (unlikely(space < skb->len)) {
			...
			spin_unlock_irqrestore(&pld->tx_rx_lock, flags);
			mif_info("%s: %s qsize[%u] in[%u] out[%u] free[%u] < len[%u]\n",
...
struct link_device *pld_create_link_device(struct platform_device *pdev)
{
	...
	/* Retrieve DPRAM IRQ# */
	if (!dpram->dpram_irq) {
		dpram->dpram_irq = platform_get_irq_byname(pdev, STR_DPRAM_IRQ);
		if (dpram->dpram_irq < 0) {
			mif_info("%s: ERR! platform_get_irq_byname fail\n",
				ld->name);
			goto err;
		}
	}
	pld->irq = dpram->dpram_irq;

	/* Retrieve DPRAM IRQ flags */
	if (!dpram->dpram_irq_flags)
		dpram->dpram_irq_flags = (IRQF_NO_SUSPEND | IRQF_TRIGGER_LOW);
	pld->irq_flags = dpram->dpram_irq_flags;
