...
static int mdm6600_boot_on(struct modem_ctl *mc)
{
	...
	if (!mc->gpio_flm_uart_sel) {
		pr_err("[MSM] no gpio_flm_uart_sel data\n");
		return -ENXIO;
	}

	if ((!mc->gpio_usb_boot_en) || (!mc->gpio_boot_sw_sel)) {
		pr_err("[MSM] no gpio_usb_boot_en or gpio_boot_sw_sel. Boot is failed\n");
		return -ENXIO;
	}
	...
	gpio_direction_output(mc->gpio_boot_sw_sel, 0);
	s3c_gpio_setpull(mc->gpio_boot_sw_sel, S3C_GPIO_PULL_NONE);
	gpio_set_value(mc->gpio_boot_sw_sel, 0);

	gpio_direction_output(mc->gpio_usb_boot_en, 0);
	s3c_gpio_setpull(mc->gpio_usb_boot_en, S3C_GPIO_PULL_NONE);
	gpio_set_value(mc->gpio_usb_boot_en, 0);

	msleep(100);

	gpio_direction_output(mc->gpio_usb_boot_en, 1);
	gpio_set_value(mc->gpio_usb_boot_en, 1);

	pr_info("[MSM] <%s> USB_BOOT_EN:[%d]\n", __func__,
		gpio_get_value(mc->gpio_usb_boot_en));

	gpio_direction_output(mc->gpio_boot_sw_sel, 1);
	gpio_set_value(mc->gpio_boot_sw_sel, 1);

	pr_info("[MSM] <%s> BOOT_SW_SEL : [%d]\n", __func__,
		gpio_get_value(mc->gpio_boot_sw_sel));
...
#if defined(CONFIG_MACH_M0_CTC) || defined(CONFIG_MACH_J_CHN_CTC)
	pr_info("[MSM] <%s> BOOT_SW_SEL : [%d]\n", __func__,
		gpio_get_value(mc->gpio_boot_sw_sel));
...
int mdm6600_init_modemctl_device(struct modem_ctl *mc, struct modem_data *pdata)
{
	...
	mc->gpio_usb_boot_en = pdata->gpio_usb_boot_en;
	mc->gpio_boot_sw_sel = pdata->gpio_boot_sw_sel;
#endif
